...
/**
 * Due Refunds
 * @link http://gaymine.dyndns-free.com/projects/rnam/wiki/DueDeposits
 */

class Financial_Library_Reports_DueRefunds extends ZFReport_Library_Base
{
	 ...
	 * @var const
	 */
	const CACHEIDENTIFIER='dueRefunds';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='dueRefundsReport';
	 ...
	 * @var array
	 */
	public static $columnMap=array('unitNumber','tenantName','feeName','leaseFeeAmount','forfeitedAmount','refundedAmount');
	...
	private function query()
	{
		...
		// this query is really nasty....but here goes...
		// grabs deposits for leases that have a last day of today or prior - includes cancelled leases because they might have deposits needed refunding		
		
		$query = "SELECT `U`.`number` AS `unitNumber`,
		                 CONCAT(`US`.`firstName`,' ',`US`.`lastName`) AS tenantName,
				 `F`.`name` AS feeName,
				 `LF`.`amount` AS leaseFeeAmount,
				  (SELECT IFNULL(SUM(amount),'0.00')
				   FROM forfeitedFee AS FF
				   WHERE FF.billId=LF.billId) AS forfeitedAmount,
				   (SELECT IFNULL(SUM(amount),'0.00')
				   FROM refund AS R
				   WHERE R.billId=LF.billId) AS refundedAmount				
                             FROM `unit` AS `U`			     
			     INNER JOIN `unitModel` AS `UM` ON U.unitModelId=UM.id
                             INNER JOIN `lease` AS `L` ON U.id=L.unitId
			     INNER JOIN `tenant` AS `T` ON T.leaseId=L.id
			     INNER JOIN `leaseFee` AS `LF` ON LF.leaseId=L.id
			     INNER JOIN `bill` AS `B` ON LF.billId=B.id
			     INNER JOIN `fee` AS `F` ON LF.feeId=F.id
			     INNER JOIN `user` AS `US` ON T.userId=US.id			     
			     WHERE ((L.leaseStartDate<NOW() AND L.lastDay<=NOW() ) )
			     AND B.isPaid=1
			     AND F.refundable=1			    
			     AND T.userId NOT IN ( SELECT US.id
					           FROM `unit` AS `U`                 
						   INNER JOIN `lease` AS `L` ON U.id=L.unitId
						   INNER JOIN `tenant` AS `T` ON T.leaseId=L.id                 
						   INNER JOIN `user` AS `US` ON T.userId=US.id
						   WHERE L.leaseStartDate>NOW())
			     GROUP BY tenantName, feeName, forfeitedAmount, refundedAmount
			     HAVING (leaseFeeAmount - forfeitedAmount - refundedAmount )>0"; 		 	   		
	...
	public function exportReport()
	{
		...
		*/
		...
		$worksheet = &$workbook->addWorksheet('Due Refunds');
