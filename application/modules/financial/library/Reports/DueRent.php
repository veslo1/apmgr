...
class Financial_Library_Reports_DueRent extends ZFReport_Library_Base
{
	 ...
	 * @var const
	 */
	const CACHEIDENTIFIER='dueRent';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='dueRentReport';
	 ...
	 * @var array
	 */
	public static $columnMap=array('unitNumber','rentAmountDue','rentDueDate','numDaysUntilDue','tenantName');
	...
	private function query()
	{
		...
		->join(array('LS'=>'leaseSchedule'),'LS.leaseId=L.id',array())
		->join(array('B'=>'bill'),'LS.billId=B.id',array('rentAmountDue'=>'B.originalAmountDue','rentDueDate'=>"DATE_FORMAT(B.dueDate,'%m/%d/%Y')", 'numDaysUntilDue'=>'DATEDIFF(B.dueDate,NOW())'))		
		...
		->where('B.dueDate>=NOW() AND DATEDIFF(B.dueDate,NOW())<=30 AND B.isPaid=0');				
	...
	{
		$cache = $this->cache->getCache(Financial_Library_Reports_DueRent::CACHEIDENTIFIER);
	...
	public function exportReport()
	{
		...
		$worksheet = &$workbook->addWorksheet('Due Rent');
