...
/**
 * Balance Sheet
 * @link http://gaymine.dyndns-free.com/projects/rnam/wiki/BalanceSheet
 */

class Financial_Library_Reports_BalanceSheet extends ZFReport_Library_Base
{
   ...
   * @var const
   */
  const CACHEIDENTIFIER='balanceSheet';

  /**
   * The default seed for cache
   * @var const
   */
  const DEFAULTSEED='balanceSheetReport';
  ...
  private function query()
  {
    ...
    $result['dateTo'] = $to;    
    $result['assets'] = $at->getBalancesByAccountType('asset');
    $result['liabilities'] = $at->getBalancesByAccountType('liability');
    $result['equity'] = $at->getBalancesByAccountType('stockholderEquity');
    $result['sumAssets'] = $this->sumAccountBalance($result['assets']);
    $result['sumLiabilities'] = $this->sumAccountBalance($result['liabilities']);
    $result['sumEquity'] = $this->sumAccountBalance($result['equity']);
    $result['sumLiabilityAndEquity'] = $result['sumLiabilities'] + $result['sumEquity'];    
  ...
  public function exportReport()
  {
    ...
    $writerImpl->setSheetName($translator->translate('balanceSheet'));
