<?php
/**
 * Paid fess will display the applicants that paid the fee.
 * The report will display the date of the payment.
 * For more details, @see http://gaymine.dyndns-free.com/projects/rnam/wiki/Applicants_that_paid_the_application_fee
 * This implementation is similar to Lease Agent View , except that we won't filter by status.
 * @author Jorge Omar Vazquez <jvazquez@debserverp4.com.ar>
 */
class Applicant_Library_Reports_PaidFees extends ZFReport_Library_Base
{
	/**
	 * Identifier of the backendCache with the Zend_Cache_Manager
	 * @var const
	 */
	const CACHEIDENTIFIER='applicantpaidfees';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='applicantpaidFees';
	 ...
	 * @var array
	 */
	public static $columnMap=array('applicantName','datePaid','unitNumber','unitName','amountPaid','totalAmount','paymentType');
	...
	private function query()
	{
		...
		->join(array('AFB'=>'applicantFeeBill'),'A.id=AFB.applicantId',array())
		->join(array('B'=>'bill'),'AFB.billId=B.id',array('datePaid'=>"DATE_FORMAT(B.dateCreated,'%m/%d/%Y')"))
		->join(array('AA'=>'applicantAppliance'),'A.id=AA.applicantId',array())
		->join(array('T'=>'transaction'),'US.id=T.userId',array())
		->join(array('P'=>'payment'),'T.id=P.transactionId',array())
		->join(array('PD'=>'paymentDetail'),'P.paymentDetailId=PD.id',array('amountPaid'=>'PD.totalAmount','paymentType'=>'PD.paymentType'))
