<?php
/**
 * Created on Dec 29, 2010 by rnelson
 * @name apmgr
 * @package application.modules.financial.controllers
 * <p>
 * Lease wizard to select lease fees for inclusion with the lease 
 * </p>
 */
class Unit_Form_SelectLeaseFee extends Zend_Form {
	...
	public function init() {
		...
		$this->setMethod('post');		
		$this->setLegend('leaseWizardLeaseFees');
		...
		for( $i=0; $i < $this->count; $i++ ) {
			...
			if( $fees )  {
				...
				foreach ( $fees as $id=>$fee ) { 
				    $this->getElement($txtId)->addMultiOption($fee['leaseFeeId'], $fee['feeName'] . ' - ' . $fee['amount'] );
				}
			}
