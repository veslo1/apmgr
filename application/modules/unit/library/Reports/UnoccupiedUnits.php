...
/**
 * Unoccupied Units
 * @link http://gaymine.dyndns-free.com/projects/rnam/wiki/UnoccupiedUnits
 */

class Unit_Library_Reports_UnoccupiedUnits extends ZFReport_Library_Base
{
	 ...
	 * @var const
	 */
	const CACHEIDENTIFIER='unoccupiedUnits';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='unoccupiedUnitsReport';
	 ...
	 * @var array
	 */
	public static $columnMap=array('unitNumber','unitModel','baseRentAmount','lastOccupancyDay','nextOccupancyDay');
	...
	private function query()
	{
		...
		$query = "SELECT `U`.`number` AS `unitNumber`,`UM`.`name` AS `unitModel`, DATE_FORMAT(MAX(L.lastDay),'%m/%d/%Y') AS `lastOccupiedDay`, DATE_FORMAT(MIN(LN.leaseStartDate),'%m/%d/%Y') AS `nextOccupiedDay`, `MRS`.`rentAmount`
                             FROM `unit` AS `U`
			     ...
			     LEFT JOIN `lease` AS `LN` ON U.id=LN.unitId AND LN.leaseStartDate>NOW() AND LN.isCancelled=0
	...
	public function exportReport()
	{
		...
		*/
		...
		$worksheet = &$workbook->addWorksheet('Unoccupied Units');
