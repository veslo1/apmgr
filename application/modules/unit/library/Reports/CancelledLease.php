...
/**
 * Cancelled Lease 
 * @link http://gaymine.dyndns-free.com/projects/rnam/wiki/CancelledLeaseReport
 */

class Unit_Library_Reports_CancelledLease extends ZFReport_Library_Base
{
	 ...
	 * @var const
	 */
	const CACHEIDENTIFIER='cancelledLease';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='cancelledLeaseReport';
	 ...
	 * @var array
	 */
	public static $columnMap=array('unitNumber','tenantName','rentAmount','leaseEffectiveDate','leaseEndDate', 'cancellationDate');
	...
	private function query()
	{
		...
		$query->from(array('L'=>'lease'),array('leaseStartDate'=>"DATE_FORMAT(L.leaseStartDate,'%m/%d/%Y')",'leaseEndDate'=>"DATE_FORMAT(L.leaseEndDate,'%m/%d/%Y')",'lastDay'=>"DATE_FORMAT(L.lastDay,'%m/%d/%Y')",'cancellationDate'=>"DATE_FORMAT(L.cancellationDate,'%m/%d/%Y')"))		
			->join(array('MRS'=>'modelRentScheduleItem'),'MRS.id=L.modelRentScheduleItemId',array('rentAmount'=>'MRS.rentAmount'))
			->join(array('U'=>'unit'),'L.unitId = U.id',array('unitNumber'=>'U.number'))
			->join(array('T'=>'tenant'),'L.id=T.leaseId',array('T.userId'))
			->join(array('US' => 'user'),'T.userId=US.id',array('tenantName'=>"CONCAT_WS(' ',US.firstName, US.lastName)"))
			->where('isCancelled=1');
			//->where('DATE(NOW()) BETWEEN DATE(L.leaseStartDate) and (L.lastDay)');
	...
	public function exportReport()
	{
		...
		$worksheet = &$workbook->addWorksheet('Cancelled Lease');
