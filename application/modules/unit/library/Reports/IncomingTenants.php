<?php
/**
 * Incoming Tenants Report
 * @link http://gaymine.dyndns-free.com/projects/rnam/wiki/IncomingTenants
 */

class Unit_Library_Reports_IncomingTenants extends ZFReport_Library_Base
{
	 ...
	 * @var const
	 */
	const CACHEIDENTIFIER='incomingTenants';

	/**
	 * The default seed for cache
	 * @var const
	 */
	const DEFAULTSEED='incomingTenantsReport';
	 ...
	 * @var array
	 */
	public static $columnMap=array('tenantName','unitNumber','rentAmount','leaseEffectiveDate','leaseEndDate');
	...
	{
		$result = null;
		$db = $this->getGatewayLink();			
		
		$query = "SELECT `U`.`number` AS `unitNumber`, `UM`.`name` AS `unitModelName`, DATE_FORMAT(L.leaseStartDate,'%m/%d/%Y') AS `leaseStartDate`, DATE_FORMAT(L.leaseEndDate,'%m/%d/%Y') AS `leaseEndDate`, `MRS`.`rentAmount`, CONCAT(`US`.`firstName`,' ',`US`.`lastName`) AS tenantName
                             FROM `unit` AS `U`
			     ...
			     WHERE ((L.leaseStartDate>=NOW() AND L.leaseEndDate>=NOW() ) AND (L.isCancelled=0)) 
			     AND T.userId NOT IN ( SELECT US.id
					           FROM `unit` AS `U`                 
						   INNER JOIN `lease` AS `L` ON U.id=L.unitId
						   INNER JOIN `tenant` AS `T` ON T.leaseId=L.id                 
						   INNER JOIN `user` AS `US` ON T.userId=US.id
						   WHERE L.leaseStartDate<NOW() AND L.isCancelled=0)"; 	    		
	...
	public function exportReport()
	{
		...
		$worksheet = &$workbook->addWorksheet('Incoming Tenants');
